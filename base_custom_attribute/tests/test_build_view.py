...
class BuildViewCase(SavepointCase, FakeModelLoader):
    ...
    def _check_attrset_visiblility(self, attrs, set_ids):
        ...
        self.assertEqual(set(set_ids), set(domain[2]))

    def _get_attr_element(self, name):
        views = self.env["res.partner"]._build_attribute_view()
        return views.find("group/*[@name='{}']".format(name))
    ...
    def test_group_visibility(self):
        ...
        views = self.env["res.partner"]._build_attribute_view()
        group = views.getchildren()[0]
        self._check_attrset_visiblility(group.get("attrs"), [self.set_1.id, self.set_2.id])

    def test_attribute_order(self):
        views = self.env["res.partner"]._build_attribute_view()
        attrs = [item.get("name") for item in views.getchildren()[0].getchildren()]
        self.assertEqual(attrs, ["x_attr_1", "x_attr_2"])

        cls.attr_1.sequence = 3
        views = self.env["res.partner"]._build_attribute_view()
        attrs = [item.get("name") for item in views.getchildren()[0].getchildren()]
    ...
    def test_attr_visibility(self):
        ...
        self.attr_1.attribute_set_ids = self.set_2
        attrs = self._get_attr_element("x_attr_1").get("attrs")
        self._check_attrset_visiblility(attrs, [self.set_2.id])
    ...
    def test_render(self):
        ...
        for attr_type, _name in field.selection:
            ...
            if attr_type == "text":
                self.assertEqual(
                    attr.tag, "group", "Type {} should be rendered as an Group")
            else:
                self.assertEqual(
                    attr.tag, "field", "Type {} should be rendered as an Field")
