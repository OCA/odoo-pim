...
class TestAttributeSetCompleteness(SavepointComponentCase):
    ...
    def test_completion_rate(self):
        ...
        self.assertFalse(partner.is_completion_state_visible)
        self.assertFalse(partner.is_completion_rate_visible)

        partner.write({"attribute_set_id": self.attr_set.id})
        partner.invalidate_cache()
        self.assertEqual(partner.completion_state, "not_complete")
        self.assertEqual(partner.completion_rate, 0.0)
        self.assertTrue(partner.is_completion_state_visible)
        self.assertTrue(partner.is_completion_rate_visible)
        ...
        self.attr_set.write({"attribute_set_completeness_ids": [(5, 0, 0)]})
        partner.write({"attribute_set_id": self.attr_set.id})
        self.assertEqual(partner.completion_state, "not_complete")
        self.assertEqual(partner.completion_rate, 0.0)
        self.assertFalse(partner.is_completion_state_visible)
